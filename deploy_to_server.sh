...
ssh -tt $SERVER_USERNAME@$SERVER_IP << END
    ...
    echo $SERVER_PASS sudo systemctl reload gunicorn_kodjaz.service
    echo $SERVER_PASS sudo systemctl daemon-reload
    echo $SERVER_PASS sudo systemctl reload nginx
    echo "Exiting"
    exit
END
